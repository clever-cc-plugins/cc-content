...
X's algorithm weights replies ~27x likes, making conversation-inviting posts outperform
like-optimized posts.
...
| **Ideal length**   | N/A         | 5-7 posts (~70% completion rate at this length)                                                        |
...
- **Hashtags**: Use **0–2 maximum**. Hashtags have minimal impact on X's current ranking.
  Using 3+ hashtags triggers spam filtering. (This is a departure from historical
  5–10 hashtag conventions on other platforms.)
- **Links**: Posts containing off-platform links see reach reduced 50–90% (non-Premium
  accounts can see near-zero median engagement since March 2026). Consider omitting links
  in favor of native content, or use exclusively with Premium accounts.
- **Emojis**: Use 1–2 emojis max per post; reserved for punctuation and emphasis, not
  bullet substitutes. Place at natural pauses.
- **Video**: Native video gets roughly **10x the engagement of text-only posts**.
  Prioritize video for awareness and reach-focused goals.

### What Separates High from Low Performers

- **Replies** are weighted ~27x likes in ranking signals; posts that explicitly invite
  conversation (e.g. "What's your biggest X pain point?") outperform like-optimized posts.
- **Native content** (video, images, threads) dramatically outperforms text-only posts; links
  dramatically reduce reach unless the poster is Premium.
- **Out-of-network reach** on X's algorithm gives early-engagement-velocity boosts to posts
  from new or cold audiences. High early replies/retweets push a post wider even if the
...
| ----------------- | ----------------------- | ---------------------- | ---------------------------- | -------------------------------------- | ------------------------- |
| **Awareness**     | 280 chars (single post) | Visual/stat            | Moderate (retweet for reach) | Surprising stat or bold claim          | Low (pure reach focus)    |
| **Consideration** | Thread (5–7 posts)      | Deep explanation       | Moderate (conversation)      | Problem hook + value promise           | Medium (educate & engage) |
...
| **Advocacy**      | 280 chars (single post) | Social proof           | Moderate (share pattern)     | User wins or peer success              | Medium (amplify wins)     |
| **Education**     | Thread (5–7 posts)      | Examples, steps        | Low (conversation)           | Problem recognition                    | Low (value-driven)        |

⚠ KNOWLEDGE-BASED for goal-by-goal breakdown — sources establish universal mechanics
(hooks, CTAs, reply-weighting, thread structure) but do not enumerate X-specific tactics
per goal. Adapt universal-structure findings: awareness favors video/visual + broad hook;
consideration favors thread-form explainers; conversion favors single clear CTA post
(not thread, since threads dilute a single action's prominence); advocacy leans on the
retweet-to-share CTA pattern.
...
| ----------------- | --------------------------- | ----------------- | ------------------- | --------------------- | -------------------------- | --------------------- | -------------------------------- |
| **TOFU (Top)**    | Accessible, problem-focused | Broad/provocative | Story or stat       | Low (follow for more) | 280 chars (single)         | Unaware, passive      | Too technical or feature-focused |
...
- [ ] Links omitted or posted only if account is Premium
...
- [ ] If thread: 5–7 posts max, with hook post (1/N), body posts (one idea each), CTA post
- [ ] Conversation-inviting elements present (question, retweet request, or similar)
